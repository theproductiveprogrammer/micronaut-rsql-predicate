package com.charleslobo.micronaut;
...
public class RsqlCriteriaBuilder {
     ...
     */
    private <T> Predicate buildPredicate(Node node, Root<T> root, CriteriaBuilder cb, Class<T> entityClass) {
        if (node instanceof AndNode) {
            List<Predicate> children = ((AndNode) node).getChildren().stream()
                    .map(child -> buildPredicate(child, root, cb, entityClass))
                    .collect(Collectors.toList());
            return cb.and(children.toArray(new Predicate[0]));
        } else if (node instanceof OrNode) {
            List<Predicate> children = ((OrNode) node).getChildren().stream()
                    .map(child -> buildPredicate(child, root, cb, entityClass))
                    .collect(Collectors.toList());
     ...
     */
    private <T> Predicate comparisonNodeToPredicate(ComparisonNode comparison, Root<T> root, CriteriaBuilder cb, Class<T> entityClass) {
        String fieldName = comparison.getSelector();
        Object value = convertValue(comparison.getArguments().get(0), getFieldType(entityClass, fieldName));

        switch (comparison.getOperator().getSymbol()) {
            case "==": return cb.equal(root.get(fieldName), value);
            case "!=": return cb.notEqual(root.get(fieldName), value);
            case ">": return cb.greaterThan(root.get(fieldName), (Comparable) value);
            case "<": return cb.lessThan(root.get(fieldName), (Comparable) value);
            case ">=": return cb.greaterThanOrEqualTo(root.get(fieldName), (Comparable) value);
            case "<=": return cb.lessThanOrEqualTo(root.get(fieldName), (Comparable) value);
            case "=like=": return cb.like(root.get(fieldName), value.toString());
            default:
                throw new UnsupportedOperationException("Operator not supported: " + comparison.getOperator().getSymbol());
        }
    }
